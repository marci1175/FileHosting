...
use tokio::{net::windows::named_pipe, sync::mpsc::{self, Receiver}, task::futures};
...
impl Serving for FileService {
    async fn provide_file(
        ...
        if msg_password == self.password {
            ...
            return Ok(Response::new(FileReply {
                serialized_file: ServerReply {file: None, error: Some("Invalid password".to_string()) }.serialize()
            }));
        }
...
impl Communication for FileService {
    async fn provide_list(&self, request: Request<Ping>) -> Result<Response<FileList>, Status> {
        if self.password == request.into_inner().password {
            ...
            Ok(Response::new(FileList {
                list: ServerPing {file: None, error: None }.serialize(),
            })) 
        }
    }
}
...
pub async fn server_spawner(password: String, port: i64, signal: Receiver<()>, file_list: Vec<PathItem>) -> anyhow::Result<()> {
